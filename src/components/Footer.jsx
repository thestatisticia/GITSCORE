...
export default function Footer({ onThemeToggle, theme }) {
  ...
  return (
    <footer className="mt-12 border-t border-white/5">
      <div className="max-w-6xl mx-auto px-4 py-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <p className="section-title">GSCORE</p>
          <p className="text-gray-500 text-sm">Trustless GitHub scoring on Flare</p>
        </div>
          ...
          <div className="flex items-center gap-3">
            {socials.map((social) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-sm hover:bg-white hover:text-black transition-colors"
              >
                {social.icon}
              </a>
            ))}
